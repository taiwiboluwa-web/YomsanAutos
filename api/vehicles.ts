...
async function ensureDatabase() {
  ...
  if (Number(count[0]?.count) === 0) {
    for (const v of seedVehicles) {
      await sql`INSERT INTO public.vehicles (id,brand,model,year,type,status,price,mileage,engine,hp,hero,gallery,colors,wheels,interiors) VALUES (${v.id},${v.brand},${v.model},${v.year},${v.type},${v.status},${v.price},${v.mileage},${v.engine},${v.hp},${v.hero},${JSON.stringify(v.gallery)},${JSON.stringify(v.colors)},${JSON.stringify(v.wheels)},${JSON.stringify(v.interiors)}) ON CONFLICT (id) DO NOTHING`;
    }
  }
  return sql;
}
...
export async function PUT(request: Request) {
  try {
    if (!auth(request)) return Response.json({ error: process.env.ADMIN_PASSWORD ? 'Unauthorized' : 'Admin backend is not configured yet. Add ADMIN_PASSWORD to Vercel.' }, { status: 401 });
    const body = await request.json();
    if (!Array.isArray(body.vehicles)) return Response.json({ error: 'Invalid inventory payload.' }, { status: 400 });
    const sql = await ensureDatabase();
    await sql`DELETE FROM public.vehicles`;
    ...
    return Response.json({ ok: true, source: 'neon', count: body.vehicles.length }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : 'Inventory save failed.' }, { status: 500 });
  }
}
