...
export default function App() {
  ...
  const heroVehicle = vehicles[activeVehicle];
    ...
    <main className="yomsan">
      <header className="nav">
        ...
        <nav className={menuOpen ? 'nav-links open' : 'nav-links'}>
          <a href="#collection" onClick={() => setMenuOpen(false)}>Collection</a><a href="#experience" onClick={() => setMenuOpen(false)}>Experience</a><a href="#contact" onClick={() => setMenuOpen(false)}>Contact</a>
        </nav>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero__copy"><p className="eyebrow">YOMSAN AUTOMOTIVE / LAGOS</p><h1>AUTOMOTIVE.<br /><i>REDEFINED.</i></h1><p className="hero__lead">A private digital showroom for exceptional automobiles. Explore the collection, shape your specification and start a conversation with YOMSAN.</p><button className="hero-cta" onClick={() => document.getElementById('collection')?.scrollIntoView({ behavior: 'smooth' })}>Explore collection <ArrowDown size={17} /></button></div>
        <div className="hero__meta"><span>{String(activeVehicle + 1).padStart(2, '0')}</span><span>{String(vehicles.length).padStart(2, '0')}</span><div className="progress"><i style={{ width: `${((activeVehicle + 1) / vehicles.length) * 100}%` }} /></div></div>
      </section>

      <section className="intro" id="experience"><div><p className="eyebrow">THE YOMSAN STANDARD</p><h2>Not a catalogue.<br /><i>A showroom.</i></h2></div><p>Every vehicle deserves more than a thumbnail and a price tag. YOMSAN brings the theatre of a private showroom to the screen — considered photography, decisive typography and an interface built around the vehicle.</p></section>

      <section className="feature" aria-label="Featured vehicle"><div className="feature__image"><img src={vehicles[0].hero} alt="Featured YOMSAN vehicle" /><span>01 / FEATURED</span></div><div className="feature__copy"><p className="eyebrow">THE PERFORMANCE COLLECTION</p><h2>{vehicles[0].brand}<br /><i>{vehicles[0].model}</i></h2><p>{vehicles[0].hp} horsepower. {vehicles[0].engine}. {vehicles[0].mileage.toLocaleString()} miles. A flagship performance sedan selected for drivers who refuse ordinary.</p><button onClick={() => setSelected(vehicles[0])}>Configure this vehicle <ArrowUpRight /></button></div></section>

      <section className="collection" id="collection"><div className="section-head"><div><p className="eyebrow">CURATED / {String(vehicles.length).padStart(2, '0')} VEHICLES</p><h2>The collection</h2></div><p>Swipe, select, configure. Your next drive starts here.</p></div><div className="vehicle-grid">{vehicles.map((vehicle) => <motion.article className="vehicle-card" key={vehicle.id} whileHover={{ y: -6 }} onClick={() => setSelected(vehicle)}><div className="vehicle-card__image"><img src={vehicle.hero} alt={`${vehicle.brand} ${vehicle.model}`} /><span>{vehicle.status}</span></div><div className="vehicle-card__body"><div><small>{vehicle.year} / {vehicle.type}</small><h3>{vehicle.brand} <i>{vehicle.model}</i></h3></div><strong>{formatPrice(vehicle.price)}</strong></div><div className="vehicle-card__footer"><span>{vehicle.hp} HP</span><span>{vehicle.mileage.toLocaleString()} MI</span><button aria-label={`Configure ${vehicle.model}`}><ArrowUpRight size={16} /></button></div></motion.article>)}</div></section>
