...
export function VehicleConfigurator({ vehicle, onClose }: Props) {
  ...
  const image = useMemo(() => {
    ...
    return colorImage || vehicle.gallery[galleryIndex];
  }, [vehicle, galleryIndex, color]);

  const whatsapp = `https://wa.me/2348033090335?text=${encodeURIComponent(`Hello YOMSAN, I am interested in the ${vehicle.year} ${vehicle.brand} ${vehicle.model}. Please send me availability and viewing details.`)}`;
  ...
  return (
    <motion.section className="configurator" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="configurator__visual">
        <div className="configurator__topbar">
          <button className="ghost-button" onClick={onClose}><ChevronLeft size={17} /> Collection</button>
          <span>YOMSAN / {vehicle.brand}</span>
        </div>
        <motion.img key={image} src={image} alt={`${vehicle.brand} ${vehicle.model}`} initial={{ opacity: .2, scale: 1.03 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .45 }} />
        ...
        <div className="gallery-controls">
          <button onClick={prev} aria-label="Previous image"><ChevronLeft /></button>
          <span>{String(galleryIndex + 1).padStart(2, '0')} / {String(vehicle.gallery.length).padStart(2, '0')}</span>
          <button onClick={next} aria-label="Next image"><ChevronRight /></button>
        </div>
      </div>

      <aside className="configurator__panel">
        <div className="panel-heading">
          <p>BUILD YOUR YOMSAN</p>
          <h1>{vehicle.brand}<br /><em>{vehicle.model}</em></h1>
          <div className="price-row"><strong>{formatPrice(vehicle.price)}</strong><span>Estimated vehicle price</span></div>
        </div>
        ...
        <div className="spec-strip">
          <div><b>{vehicle.hp}</b><span>HP</span></div><div><b>{vehicle.engine.split(' ')[0]}</b><span>ENGINE</span></div><div><b>{vehicle.mileage.toLocaleString()}</b><span>MILES</span></div>
        </div>

        <div className="enquiry-actions">
          <a className="primary-action" href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle size={17} /> Request this vehicle <ArrowUpRight size={16} /></a>
          <div className="secondary-actions"><a href="tel:+2348033090335"><Phone size={15} /> Call YOMSAN</a><a href={`${whatsapp}&source=inspection`} target="_blank" rel="noreferrer"><CalendarDays size={15} /> Book inspection</a></div>
        </div>
      </aside>
    </motion.section>
  );
